...
utils.decodeCred = function(credString) {
    ...
    let asciiString = Buffer.from(b64String, "base64").toString("ascii"); // 'test1:hei'

    //extract the username - using regex
    cred.username = asciiString.replace(/:.*/, ""); //test1

    // extract password
    cred.password = asciiString.replace(cred.username + ":", "") //hei
